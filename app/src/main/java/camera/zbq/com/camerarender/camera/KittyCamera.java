package camera.zbq.com.camerarender.camera;
...
public class KittyCamera implements ICameraHelper {
    ...
    public boolean open(int cameraId) {
        ...
        if (mCamera != null) {
            ...
            preSize = getPropPreviewSize(param.getSupportedPreviewSizes(), mConfig.rate, mConfig
                    .minPreviewWidth);
            param.setPictureSize(preSize.width,preSize.height);
            ...
            LogTools.e("KittyCamera", "camera previewSize:" + mPreSize.x + "/" + mPreSize.y);
            return true;
        }
        return false;
    }
    ...
    public void setOnPreviewFrameCallbackWithBuffer(final PreviewFrameCallback callback) {
        if (mCamera != null) {
            LogTools.p("KittyCamera", "Camera set CallbackWithBuffer");
    ...
    public void setOnPreviewFrameCallbackWithBuffer(final PreviewFrameCallback callback,byte[]buffer) {
        if (mCamera != null) {
            mCamera.addCallbackBuffer(buffer);
            LogTools.p("KittyCamera", "Camera set CallbackWithBuffer");
            mCamera.setPreviewCallbackWithBuffer(new Camera.PreviewCallback() {
