...
public class UVCCameraHelper {
    ...
    public void initUSBMonitor(final Activity activity, CameraViewInterface cameraView, final OnMyDevConnectListener listener) {
    ...
    public void initUSBMonitor(final Context activity, final OnMyDevConnectListener listener) {
        this.mActivity = activity;
        mUSBMonitor = new USBMonitor(activity.getApplicationContext(), new USBMonitor.OnDeviceConnectListener() {
...
//            throw new NullPointerException("CameraViewInterface cannot be null!");
        ...
        if (mUSBMonitor != null) {
            ...
            if (device == null) {
                ...
                for (UsbDevice usbDevice : devices) {
                    String productN = usbDevice.getProductName();
                    if (productN != null && productN.indexOf("Camera") != -1) {
                        Log.e("zbq", "usb授权--" + usbDevice.getProductName() + "," + usbDevice.getManufacturerName());
                        mUSBMonitor.requestPermission(usbDevice);
                    }
                }

            } else {
                Log.e("zbq", "usb授权" + device.getDeviceName() + ",id:" + device.getDeviceId());
    ...
    public void startPreview(CameraViewInterface cameraView) {
        ...
        if (cameraView == null) {
            ...
            st = cameraView.getSurfaceTexture();
            if (st == null) {
                st = new SurfaceTexture(0);
            }
        }
