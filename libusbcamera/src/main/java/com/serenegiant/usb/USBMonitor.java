...
import com.jiangdg.usbcamera.utils.UsbPermissionUtils;
...
public final class USBMonitor {
    ...
    public static final String ACTION_USB_PERMISSION = ACTION_USB_PERMISSION_BASE;  //+ hashCode()
        ...
        if (!destroyed) {
            ...
            if (keys != null) {
                ...
                try {
                    ...
                    LogTools.e(TAG, e, "destroy: error");
                }
            }
            mCtrlBlocks.clear();
            try {
...
//		if (DEBUG) Log.v(TAG, "requestPermission:device=" + device);
    ...
    private final BroadcastReceiver mUsbReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(final Context context, final Intent intent) {
            ...
            if (ACTION_USB_PERMISSION.equals(action)) {
                ...
                if (!destroyed && productName != null && (productName.contains("Camera")
                        || productName.contains("camera") || productName.contains("AV") || productName.contains("WebCam"))) {
                    grantPermission(context, device);
                }
    ...
    public void grantPermission(Context context, UsbDevice device) {
        if (device != null) {
            processAttach(device);
            try {
                // if (mUsbManager != null && !mUsbManager.hasPermission(device)) {
                UsbPermissionUtils.grantAutomaticPermission(device, context);
                LogTools.p(TAG, "开始授权 e:" + device.getProductName());
                //}
                Intent intent = new Intent();
                intent.setAction(USBMonitor.ACTION_USB_PERMISSION);
                intent.putExtra(UsbManager.EXTRA_DEVICE, device);
                mWeakContext.get().sendBroadcast(intent);
                updatePermission(device, true);
            } catch (Exception e) {
                LogTools.e(TAG, "调用授权失败 e:" + e.getMessage());
        ...
        public void run() {
            ...
            if ((n > mDeviceCounts) || (m > hasPermissionCounts)) {
                for (int i = 0; i < n; i++) {
                    ...
                    if (!destroyed && productName != null && (productName.contains("Camera") ||
                            productName.contains("camera") || productName.contains("AV") || productName.contains("WebCam"))) {
                        LogTools.p(TAG, "找到usb摄像头 " + productName);
...
//                        }
    ...
    private final void processConnect(final UsbDevice device) {
        ...
        final int deviceKey = getDeviceKey(device, true);
        if (mHasPermissions.get(deviceKey) == null) {
            LogTools.p(TAG, "usb camera permission processConnect");
            updatePermission(device, true);
        }
        mAsyncHandler.post(new Runnable() {
            @Override
    ...
    private final void processDettach(final UsbDevice device) {
        ...
        if (mOnDeviceConnectListener != null) {
            mAsyncHandler.post(new Runnable() {
                @Override
                public void run() {
                    LogTools.p(TAG, "usb拔出" + device.getDeviceName() + ",id:" + device.getDeviceId());
                    updatePermission(device, false);
